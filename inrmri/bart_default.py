...
os.environ['PATH'] += ':/opt/bart/bart-0.8.00'
os.environ["TOOLBOX_PATH"] = "<path to bart folder inside NF-CMRI folder>"
...
def bart_acquisition_from_files(path, name):
  ...
  if not cfl_exists(trajpath): 
    ...
    return 
  data = bart_read(datapath)[...,None, None, None, None, None]
  traj = bart_read(trajpath)[...,None, None, None, None, None]
...
class BARTAcquisition: 
  ...
  def calculate_coil_sens(self): 
    ...
    return read_csmap(csmappath, self.name) # BartCoilSensitivityMaps(cs, csmappath, self.name) 
